...
class DataTransformation:
    ...
    def imbalanced_data_cleaning(self):
        ...
        try:
            ...
            imbalanced_data = pd.read_csv(
                self.data_ingestion_artifacts.imbalanced_data_file_path)
            ...
            logging.info(
                f"Exited the imbalanced data cleaning function and returned imbalanced data {imbalanced_data}")
    ...
    def raw_data_cleaning(self):
        ...
        try:
            logging.info("Entered raw data cleaning function")
            ...
            logging.info(
                f"Exited the raw data cleaning function and returned raw data {raw_data}")
            return raw_data
    ...
    def concat_dataframes(self):
        ...
        try:
            logging.info("Entered into the concat_dataframes function")

            # let's concatenate both the dataframes into a single dataframe
            frame = [self.raw_data_cleaning(), self.imbalanced_data_cleaning()]
            df = pd.concat(frame)
            print(df.head())
            logging.info(f"returning the concatenated dataframe:{df}")
            return df
        except Exception as e:
            raise CustomException(e, sys) from e
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifacts:
        ...
        try:
            logging.info(
                "Entering the initiate data transformation method of the DataTransformation class")
            ...
            logging.info("returning the data transformation artifacts")
